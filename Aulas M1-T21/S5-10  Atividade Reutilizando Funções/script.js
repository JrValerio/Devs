...
function letraInicial (nome, sobrenome) {
    let letraInicialDoNome = nome[0].toUpperCase();
    let letraInicialDoSobrenome = sobrenome[0].toUpperCase();
    
    return `${letraInicialDoNome} e ${letraInicialDoSobrenome}`;
}

function nomeCompleto () {
    const primeiraLetra = letraInicial("Amaro", "Júnior");

    return `As letras inicias do meu nome completo são ${primeiraLetra}`;
}
